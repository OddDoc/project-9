...
Panel_Options::Panel_Options(vgui::VPANEL parent, const char* pName) : BaseClass(NULL, "")
{
	...
	SetPanelTitle("#GameUI2_TitleOptions");
...
void Panel_Options::SetPanelTitle(const char* text)
{
	if (text[0] == '#')
	{
		...
		if (tempString)
		{
			wcsncpy(m_PanelTitle, tempString, sizeof(m_PanelTitle) / sizeof(wchar_t));
			m_PanelTitle[(sizeof(m_PanelTitle) / sizeof(wchar_t)) - 1] = 0;
		}
		else
		{
			const size_t cSizeText = strlen(text) + 1;
			mbstowcs(m_PanelTitle, text, cSizeText);
		}
	}
	else
	{
...
void Panel_Options::ApplySchemeSettings(vgui::IScheme* pScheme)
{
	...
	m_cBackgroundGradientTop = GetSchemeColor("Panel.Background.GradientTop", pScheme);
	m_cBackgroundGradientBottom = GetSchemeColor("Panel.Background.GradientBottom", pScheme);
...
{
	new Panel_Options(GameUI2().GetRootPanel(), "");
}