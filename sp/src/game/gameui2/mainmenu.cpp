...
MainMenu::MainMenu(vgui::Panel* parent) : BaseClass(NULL, "MainMenu")
{
	...
	SetLogoLeft("#GameUI2_P9LogoLeft");
	SetLogoRight("#GameUI2_P9LogoRight");
...
void MainMenu::SetLogoLeft(const char* text)
{
	if (text[0] == '#')
	{
		...
		if (tempString)
		{
			wcsncpy(m_logoLeft, tempString, sizeof(m_logoLeft) / sizeof(wchar_t));
			m_logoLeft[(sizeof(m_logoLeft) / sizeof(wchar_t)) - 1] = 0;
		}
		else
		{
			const size_t cSizeText = strlen(text) + 1;
			mbstowcs(m_logoLeft, text, cSizeText);
		}
	}
	else
	{
...
void MainMenu::SetLogoRight(const char* text)
{
	if (text[0] == '#')
	{
		wchar_t* tempString = g_pVGuiLocalize->Find(text);
		if (tempString)
		{
			wcsncpy(m_logoRight, tempString, sizeof(m_logoRight) / sizeof(wchar_t));
			m_logoRight[(sizeof(m_logoRight) / sizeof(wchar_t)) - 1] = 0;
		}
		else
		{
			const size_t cSizeText = strlen(text) + 1;
			mbstowcs(m_logoRight, text, cSizeText);
		}
	}
	else
	{
